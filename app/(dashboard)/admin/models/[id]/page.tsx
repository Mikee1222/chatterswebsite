...
import { ModelChurnRiskBadge } from "@/components/model-churn-risk-widgets";
import {
  computeModelChurnRisk,
  notifyHighChurnIfNeeded,
} from "@/services/model-churn-risk";
...
export default async function AdminModelDetailPage({ params }: { params: { id: string } }) {
  ...
  const churnRisk = await computeModelChurnRisk(model);
  void notifyHighChurnIfNeeded(churnRisk);
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-white/60">
          Model record <code className="text-white/70">{model.id}</code> · {model.platform} ·{" "}
          {model.status}
        </p>
      </div>
        ...
        >
          ...
          {isAdmin ? (
            <div className="mt-4 max-w-md">
              <AdminModelPeriodTrackingToggle
                modelId={model.id}
                enabled={model.period_tracking_enabled === true}
              />
            </div>
          ) : (
            <p className="mt-3 text-sm text-white/50">
              Period tracking toggle is visible to admins only.
            </p>
          )}
        </div>
        <ModelChurnRiskBadge risk={churnRisk} />
      </div>

      <p className="text-sm text-white/50">
        Edit account-linked fields in{" "}
        <Link
          href={ROUTES.modelEdit(model.id)}
          className="text-pink-300/90 underline-offset-2 hover:underline"
        >
          Accounts → model
        </Link>
        .
      ...
      {typeof model.of_user_id === "string" && model.of_user_id.trim() !== "" ? (
        <SyncOFSubscribersButton
          ofAccountId={model.of_user_id}
