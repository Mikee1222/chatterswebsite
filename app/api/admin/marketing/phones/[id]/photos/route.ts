...
import { getPhoneDetail, uploadPhonePhotos, appendPhonePhotoUrls } from "@/services/marketing";
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (photoUrls.length > 0) {
    for (const token of photoUrls) {
      if (!isAllowedDirectUploadToken(token, "marketing-phone-photos", { itemId: id })) {
        return NextResponse.json({ error: "Invalid photo reference" }, { status: 400 });
      }
    }
    await appendPhonePhotoUrls(id, photoUrls);
  } else {
