...
import { createExecAudit, getExecAuditsForVA } from "@/services/marketing-reviews";
...
export async function GET(req: Request) {
  ...
  const vaId = new URL(req.url).searchParams.get("va_id");
  if (!vaId?.trim()) {
    return NextResponse.json({ error: "va_id is required" }, { status: 400 });
  }
  const execAudits = await getExecAuditsForVA(vaId.trim());
  return NextResponse.json({ execAudits });
}
