    ...
    if (!hasStarted && now - scheduledMs >= LATE_THRESHOLD_MS && program.chatter_id) {
      ...
      if (!dupChatter) {
        await notify({
          ...
          priority: NOTIFICATION_PRIORITY.HIGH,
          title: "⚠️ You're late for your shift",
          body: `⚠️ Your shift was supposed to start ${minsLate} minutes ago. Please log in now.`,
          entity_type: NOTIFICATION_ENTITY.SHIFT,
      ...
      await notifyAdminsOnce(
        {
          event_type: NOTIFICATION_EVENT.SHIFT_LATE,
          priority: NOTIFICATION_PRIORITY.HIGH,
          title: `⚠️ ${chatterName} is late`,
          body: `⚠️ ${chatterName} hasn't started their shift yet — ${minsLate} minutes late.`,
          entity_type: NOTIFICATION_ENTITY.SHIFT,
