#!/usr/bin/env tsx
/**
 * Adds new font choices to link_pages.font single-select via Airtable Metadata API.
 * Falls back to typecast probe records when Meta API PATCH is rejected.
 * Idempotent: skips choices that already exist.
 *
 * Usage: npm run add:link-page-fonts
 */

import { config as loadEnv } from "dotenv";
import { existsSync, readFileSync } from "node:fs";
import { resolve } from "node:path";
import { LINK_PAGE_FONTS, LINK_PAGES_TABLE } from "../lib/link-pages-schema";
...
function resolveFontField(tables: MetaTable[]): { table: MetaTable; field: MetaField } {
...
async function patchFontChoices(
  token: string,
  baseId: string,
  tableId: string,
  fieldId: string,
...
async function addChoiceViaTypecast(token: string, baseId: string, choiceName: string): Promise<void> {
  ...
  if (!createRes.ok) {
    throw new Error(
      `typecast create font="${choiceName}" failed (${createRes.status}): ${await createRes.text()}`
    );
  }
  const created = (await createRes.json()) as { records?: Array<{ id: string }> };
  const recordId = created.records?.[0]?.id;
  if (!recordId) {
    throw new Error(`typecast create font="${choiceName}" returned no record id`);
  }
...
async function main(): Promise<void> {
  ...
  const { table, field } = resolveFontField(tables);

  const existing = field.options?.choices ?? [];
  const existingSet = choiceNamesSet(existing);
  const toAdd = LINK_PAGE_FONTS.filter((f) => !existingSet.has(f.toLowerCase()));

  if (toAdd.length === 0) {
    console.log(`All ${LINK_PAGE_FONTS.length} font choices already exist on ${LINK_PAGES_TABLE}.${FIELD_NAME}.`);
    return;
  }

  console.log(`Adding ${toAdd.length} font choice(s)...`);

  const nextChoices = [
    ...existing.map(preserveChoice),
    ...toAdd.map((name) => ({ name })),
  ];

  const patched = await patchFontChoices(token, baseId, table.id, field.id, nextChoices);
  if (patched) {
    console.log(`Added ${toAdd.length} font choice(s) via Meta API PATCH: ${toAdd.join(", ")}`);
  } else {
  ...
  const { field: updatedField } = resolveFontField(refreshed);
  const updatedSet = choiceNamesSet(updatedField.options?.choices ?? []);
  const stillMissing = LINK_PAGE_FONTS.filter((f) => !updatedSet.has(f.toLowerCase()));
  if (stillMissing.length > 0) {
    throw new Error(`Font choices still missing after sync: ${stillMissing.join(", ")}`);
  }
