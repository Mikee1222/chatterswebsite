#!/usr/bin/env tsx
/**
 * Creates `users.last_login_user_agent` (long text) via Airtable Metadata API.
 * Idempotent: if the field already exists, logs and exits 0.
 *
 * Requires: AIRTABLE_TOKEN, AIRTABLE_BASE_ID (optional wrangler.jsonc for base id)
 *
 * Usage: npm run add:last-login-user-agent-field
 */
...
const FIELD_NAME = "last_login_user_agent";
...
async function main(): Promise<void> {
  ...
  console.log(`Created field users.${FIELD_NAME} (multiline text).`);
}
