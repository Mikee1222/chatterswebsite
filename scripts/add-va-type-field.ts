#!/usr/bin/env tsx
/**
 * Creates `users.va_type` (single select: chatting, marketing, both) via Airtable Metadata API.
 * Idempotent: if the field already exists, logs and exits 0.
 *
 * Requires: AIRTABLE_TOKEN, AIRTABLE_BASE_ID (see .env / .env.local)
 *
 * Usage: npx tsx scripts/add-va-type-field.ts
 *        npm run add:va-type-field
 */
...
const FIELD_NAME = "va_type";

const FIELD_DEF: FieldDef = {
  type: "singleSelect",
  options: {
    choices: [
      { name: "chatting", color: "blueLight2" },
      { name: "marketing", color: "purpleLight2" },
      { name: "both", color: "greenLight2" },
    ],
  },
};
...
async function main(): Promise<void> {
  ...
  console.log(`Created field users.${FIELD_NAME} (single select: chatting, marketing, both).`);
}
