...
async function createTable() {
  ...
  const existing = tables.find((t) => t.name === "model_content_requests");
  if (existing) {
    console.log(`Table already exists: model_content_requests (${existing.id})`);
    return;
  }
  ...
  const body = {
    ...
    fields: [
      ...
      { name: "model_user_id", type: "singleLineText" },
      {
        ...
        options: {
          choices: [
            { name: "script", color: "blueLight2" },
            { name: "mass", color: "purpleLight2" },
            { name: "photo_set", color: "tealLight2" },
            { name: "video", color: "greenLight2" },
  ...
  const createRes = await metaFetch("/tables", {
    method: "POST",
