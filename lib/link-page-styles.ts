...
const SVG = {
  ...
  snapchat: `<svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20" aria-hidden="true"><path d="M12.166 3C9.776 3 7.64 4.876 7.64 7.52v.79l-.898.047c-.418.022-.69.218-.69.504 0 .27.231.538.666.7l.922.34-.31.9c-.16.47-.47.87-.88 1.15-.56.38-1.14.57-1.61.57-.09 0-.17-.01-.24-.02-.13.26-.18.54-.13.8.08.41.42.71.93.82l.47.1-.18.44c-.28.68-.99 1.14-1.84 1.14-.18 0-.35-.02-.5-.06.52.84 1.62 1.42 3.19 1.72l.34.07-.11.33c-.1.3-.38.5-.72.5-.16 0-.5-.04-1.03-.19-.41-.12-.84-.18-1.27-.18-.38 0-.72.05-1.01.16.31.48.96.99 2.17 1.35 1.41.42 2.88.44 3.95.44.15 0 .29 0 .43-.01 1.07 0 2.54-.02 3.95-.44 1.21-.36 1.86-.87 2.17-1.35-.29-.11-.63-.16-1.01-.16-.43 0-.86.06-1.27.18-.53.15-.87.19-1.03.19-.34 0-.62-.2-.72-.5l-.11-.33.34-.07c1.57-.3 2.67-.88 3.19-1.72-.15.04-.32.06-.5.06-.85 0-1.56-.46-1.84-1.14l-.18-.44.47-.1c.51-.11.85-.41.93-.82.05-.26 0-.54-.13-.8-.07.01-.15.02-.24.02-.47 0-1.05-.19-1.61-.57-.41-.28-.72-.68-.88-1.15l-.31-.9.922-.34c.435-.162.666-.43.666-.7 0-.286-.272-.482-.69-.504L16.36 8.31v-.79C16.36 4.876 14.556 3 12.166 3z"/></svg>`,
...
const TIKTOK_GLASS_STYLE_CSS =
  "background:rgba(255,255,255,0.1);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.25);color:#fff;text-shadow:0 1px 2px rgba(0,0,0,0.3);";
...
function inlineStyles(
  ...
  if (platform === "snapchat") {
    if (effective === "prominent") {
      return "background:#FFFC00;color:#000000;";
    }
    if (effective === "glass") {
      return `${GLASS_STYLE_CSS}color:#fffc00;`;
    }
  }

  if (platform === "tiktok" && effective === "glass") {
    return TIKTOK_GLASS_STYLE_CSS;
  }
