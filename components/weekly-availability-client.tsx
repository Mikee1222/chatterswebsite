...
import { submitAvailabilityAction, updateAvailabilityAction } from "@/app/actions/weekly-availability";
...
/** Format custom_start_time / custom_end_time for display (ISO or HH:mm). */
...
export function WeeklyAvailabilityClient({ weekStart: initialWeekStart, initialRequests }: Props) {
  ...
  React.useEffect(() => {
    ...
    if (process.env.NODE_ENV !== "production") {
      console.log("[weekly-availability client] synced initialRequests", {
        weekStart,
  ...
  React.useEffect(() => {
    if (process.env.NODE_ENV !== "production") {
      console.log("[weekly-availability client] render", {
        weekStart,
        initialRequestsCount: initialRequests.length,
        requestsCount: requests.length,
      });
    }
  }, [weekStart, initialRequests.length, requests.length]);
  ...
  const daysWithSubmission = React.useMemo(
    () => new Set(requests.map((r) => r.day)),
    [requests]
  );
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-white/40">Your schedule</p>
        <h1 className="mt-2 text-2xl font-semibold tracking-tight text-white md:text-3xl">My weekly availability</h1>
        <p className="mt-1 text-white/60">Submit when you’re available for the selected week. This is not the final schedule—admins use it when building the weekly program.</p>
      </div>
      ...
      >
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button type="button" onClick={() => router.push(weeklyAvailabilityUrl(getThisWeekMonday()))} className="rounded-xl border border-[hsl(330,80%,55%)]/40 bg-[hsl(330,80%,55%)]/15 px-4 py-2.5 text-sm font-medium text-[hsl(330,90%,75%)] hover:bg-[hsl(330,80%,55%)]/25 transition-colors">
            This week
          </button>
          <button type="button" onClick={() => goToWeek(1)} className="rounded-xl border border-white/15 bg-white/5 px-4 py-2.5 text-sm font-medium text-white/90 hover:bg-white/10 transition-colors">
            Next →
          ...
          <form onSubmit={handleSubmit} className="mt-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>Day</Label>
                {editingRequest ? (
                  <p className="mt-1 rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-3 text-[15px] text-white/90">
                    {day}
                  </p>
                ) : (
                  <Select
                  ...
                  >
                    {DAYS.map((d) => {
                      const alreadySubmitted = daysWithSubmission.has(d);
                      return (
                        <option
                          key={d}
                          value={d}
                        ...
                        >
                          {d}{alreadySubmitted ? " (already submitted)" : ""}
                        </option>
                      );
                    })}
                  </Select>
            ...
            {entryType === "availability" && shiftType === "Custom" && (
              <div className="space-y-2">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      aria-describedby={!customStart.trim() || !customEnd.trim() ? "custom-time-error" : undefined}
                    />
                  </div>
                  <div>
                ...
                {(!customStart.trim() || !customEnd.trim()) && (
                  <p id="custom-time-error" className="text-sm text-amber-300" role="alert">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {editingRequest && (
                <button
                  ...
                  className="rounded-2xl border border-white/15 bg-white/5 px-5 py-2.5 text-sm font-medium text-white/80 hover:bg-white/10 transition-colors"
                >
                  Cancel
                </button>
              )}
                ...
                entries.length > 0 ? (
                  <div key={d}>
                    ...
                    {entries.map((r) => (
                      <div
                        key={r.id}
                        className="rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 mb-2 text-sm"
                      >
                        ...
                        <div className="mt-2">
                          <button
                            ...
                            className="rounded-lg border border-white/20 bg-white/5 px-3 py-1.5 text-xs font-medium text-white/80 hover:bg-white/10 transition-colors"
                          >
                            Edit
                          </button>
                        </div>
