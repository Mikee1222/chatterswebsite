      ...
      {subTab === "content" ? (
        <div className="space-y-5">
          ...
          <div className={cn(VA_CARD, "p-4 md:p-5")}>
            ...
            {data?.stories?.error ? (
              <IgEmptyState
                title="Stories taking a break"
                detail="We couldn’t load active Stories right now — try again after the next sync."
              />
            ) : !(data?.stories?.active?.length) ? (
              <IgEmptyState
                title="No live Stories right now"
                detail="When you have active Stories up, they’ll show here. We only list what’s live — no made-up metrics."
              />
            ) : (
              <div className="space-y-3">
                <p className="text-xs text-white/45">
                  ...
                  {!data.stories.has_metrics
                    ? " · Instagram didn’t share performance numbers for these"
                    : ""}
                </p>
                    ...
                    >
                      ...
                      <div className="space-y-1 p-3 text-xs text-white/55">
                        ...
                        {data.stories?.has_metrics ? (
                          ...
                          <p className="text-white/35">Live now — metrics not provided</p>
                        )}
                        {s.permalink ? (
                          <a
                            href={s.permalink}
