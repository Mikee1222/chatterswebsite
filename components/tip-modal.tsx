...
export function TipModal({
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!modelId || !subUsername.trim() || !Number.isFinite(amount) || amount <= 0) {
      setError("Select a model, enter username, and a valid USD amount.");
      return;
    }
    if (models.length === 0) {
      ...
      >
        {success ? (
          <div className="flex min-h-[240px] flex-col items-center justify-center text-center">
            ...
            <h2 className="mt-3 text-lg font-semibold text-white">Tip logged</h2>
          </div>
        ) : (
          <form onSubmit={onSubmit} className="space-y-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h2 className="text-lg font-semibold text-white">Log a missing tip</h2>
                <p className="mt-1 text-xs text-white/45">Unlocked tip reporting</p>
              </div>
              <button
                type="button"
                onClick={onClose}
            ...
            <label className="block text-xs font-medium uppercase tracking-wide text-white/45">
              ...
              <div className="relative mt-1.5">
                <span className="pointer-events-none absolute left-3 top-1/2 z-[1] -translate-y-1/2 text-white/55">$</span>
                <input
                  type="number"
                  min={0.01}
                  step={0.01}
                  ...
                  placeholder="0.00"
                  className="w-full rounded-xl border border-white/10 bg-white/5 py-2.5 pl-8 pr-4 text-white placeholder:text-white/35"
                  required
                />
              </div>
            </label>
