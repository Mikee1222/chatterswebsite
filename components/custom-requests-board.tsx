...
const SORT_SELECT_OPTIONS: { value: SortOption; label: string }[] = [
...
const filterDateInputClass =
  "h-11 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white outline-none transition [color-scheme:dark] focus:border-pink-500/40 focus:ring-2 focus:ring-pink-500/20";
...
export function CustomRequestsBoard({
  ...
  const modelSelectOptions = React.useMemo(
    () => [{ value: "all", label: "All models" }, ...modelOptions.map(([id, name]) => ({ value: id, label: name }))],
    [modelOptions]
  );
  const chatterSelectOptions = React.useMemo(
    () => [{ value: "all", label: "All chatters" }, ...chatterOptions.map(([id, name]) => ({ value: id, label: name }))],
    [chatterOptions]
  );
  ...
  const activeTab = isVa ? "border-sky-400/60 bg-sky-500/20 text-sky-100" : "";
  const inactiveTab = "border-white/15 bg-white/5 text-white/70 hover:bg-white/10";
  const adminQuickTabActive =
    "border border-pink-500/30 bg-gradient-to-r from-pink-500/20 to-rose-500/20 text-white shadow-sm shadow-pink-950/15";
  const adminQuickTabInactive = "border border-white/10 bg-white/5 text-white/50 hover:border-white/20 hover:text-white";
  const typeChipActiveAdmin = "border border-pink-500/50 bg-pink-500/20 text-pink-400";
  const typeChipActiveVa = "border-sky-400/55 bg-sky-500/20 text-sky-100 border";
  const typeChipInactive = "border border-white/10 bg-white/5 text-white/60 hover:border-white/20 hover:text-white";
  const exportBtn = isVa ? "border-sky-500/40 bg-sky-500/15 text-sky-100 hover:bg-sky-500/25" : "border border-white/20 bg-white/10 text-white/70 hover:bg-white/15";
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-2xl border border-t border-r border-b border-white/10 border-l-4 border-l-amber-500 bg-white/5 p-5 transition hover:bg-white/[0.07]">
          <p className="text-xs font-semibold uppercase tracking-widest text-white/40">Pending</p>
          <p className="mt-1 text-4xl font-bold tabular-nums text-white">{stats.pending}</p>
          <p className="mt-1 text-xs text-white/40">In this list</p>
        </div>
        <div className="rounded-2xl border border-t border-r border-b border-white/10 border-l-4 border-l-blue-500 bg-white/5 p-5 transition hover:bg-white/[0.07]">
          <p className="text-xs font-semibold uppercase tracking-widest text-white/40">Approved</p>
          <p className="mt-1 text-4xl font-bold tabular-nums text-white">{stats.approved}</p>
          <p className="mt-1 text-xs text-white/40">Waiting for model</p>
        </div>
        <div className="rounded-2xl border border-t border-r border-b border-white/10 border-l-4 border-l-purple-500 bg-white/5 p-5 transition hover:bg-white/[0.07]">
          <p className="text-xs font-semibold uppercase tracking-widest text-white/40">Scheduled</p>
          <p className="mt-1 text-4xl font-bold tabular-nums text-white">{stats.scheduled}</p>
          <p className="mt-1 text-xs text-white/40">Model scheduled</p>
        </div>
        <div className="rounded-2xl border border-t border-r border-b border-white/10 border-l-4 border-l-green-500 bg-white/5 p-5 transition hover:bg-white/[0.07]">
          <p className="text-xs font-semibold uppercase tracking-widest text-white/40">Completed</p>
          <p className="mt-1 text-4xl font-bold tabular-nums text-white">{stats.completed}</p>
          <p className="mt-1 text-xs text-white/40">Uploaded</p>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-sm">
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          {(["video", "photo", "custom", "other"] as TypeFilterValue[]).map((t) => (
            ...
            >
              <span className="text-sm font-medium text-white/80">{humanType(t)}</span>
      ...
      <section className="rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur-sm">
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          <div className="flex flex-col">
            <span className="mb-1 text-xs text-white/40">Status</span>
              ...
              {STATUS_FILTER_CHIPS.map(({ value, label }) => {
                ...
                return (
                  <button
                    key={value}
                    type="button"
                    onClick={() => toggleMultiSelect(statusFilter, value, setStatusFilter)}
                    className={cn(
          ...
          <div className="flex flex-col">
            <span className="mb-1 text-xs text-white/40">Model</span>
            <CustomSelect
              value={modelFilter}
          ...
          <div className="flex flex-col">
            <span className="mb-1 text-xs text-white/40">Chatter</span>
            <CustomSelect
              value={chatterFilter}
          ...
          <div className="flex flex-col">
            <span className="mb-1 text-xs text-white/40">Type</span>
            <div className="flex flex-wrap gap-2">
              {(["video", "photo", "custom", "other"] as TypeFilterValue[]).map((t) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      active ? (isVa ? typeChipActiveVa : typeChipActiveAdmin) : typeChipInactive
                    )}
                  >
                    {humanType(t)}
                  </button>
          ...
          <div className="flex flex-col">
            <span className="mb-1 text-xs text-white/40">Date range</span>
          ...
          <div className="flex flex-col md:col-span-2 xl:col-span-1">
            <span className="mb-1 text-xs text-white/40">Fan username</span>
            <div className="relative">
              ...
              <input
                ...
                className={cn(filterDateInputClass, "pl-10")}
              />
            </div>
          </div>
        </div>
        ...
        <div className="mt-5 flex justify-end border-t border-white/[0.08] pt-4">
          <button
            ...
            className="rounded-xl border border-transparent px-3 py-2 text-xs font-semibold uppercase tracking-wide text-white/50 transition hover:border-white/10 hover:bg-white/5 hover:text-white/80"
          >
            Clear filters
          </button>
        </div>
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.02] p-4">
        <div className="mb-4 flex flex-wrap items-center gap-2 rounded-2xl border border-white/10 bg-white/5 p-3">
          <button
            type="button"
            disabled={bulkBusy}
            onClick={() => void runBulkApprove()}
            className="rounded-lg border border-green-500/30 bg-green-500/20 px-3 py-2 text-sm font-medium text-green-400 transition hover:bg-green-500/30 disabled:opacity-50"
          >
            Bulk approve
          </button>
          <button
          ...
          <button
            ...
            className="rounded-lg border border-white/10 bg-transparent px-3 py-2 text-sm text-white/45 transition hover:border-white/15 hover:bg-white/5 hover:text-white/70 disabled:cursor-not-allowed disabled:opacity-40"
          ...
          <button
            type="button"
            onClick={exportCsv}
            className={cn("rounded-lg border px-3 py-2 text-sm font-medium transition", exportBtn)}
          >
            Export CSV
          </button>
        ...
        {filteredRows.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-2xl border border-white/10 bg-white/[0.03] px-6 py-16 text-center">
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-white/10 bg-white/5 text-white/30">
              <Inbox className="h-7 w-7" aria-hidden />
            </div>
            <p className="mt-5 text-sm font-medium text-white/40">No matching requests</p>
            <p className="mt-2 max-w-xs text-xs leading-relaxed text-white/35">Adjust your filters or create a new task.</p>
            <button
              type="button"
              onClick={clearFilters}
              className="mt-4 text-xs font-semibold uppercase tracking-wide text-pink-400/90 underline-offset-4 transition hover:text-pink-300 hover:underline"
            >
              Clear filters
            </button>
          </div>
            ...
            {filteredRows.map((row) => {
              ...
              return (
                <article
                  key={row.id}
                  className={cn(
                    "rounded-2xl border border-white/10 bg-zinc-950/70 p-4 transition-all duration-200",
                    cardHover
                  )}
                >
