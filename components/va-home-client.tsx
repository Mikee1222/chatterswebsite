  ...
  return (
    ...
    >
      ...
      <div className="grid gap-6 lg:grid-cols-5 lg:gap-8">
        ...
        <motion.section
          variants={sectionItem}
          className="lg:col-span-3 overflow-hidden rounded-2xl border border-white/10 bg-black/35 shadow-[0_0_0_1px_rgba(255,255,255,0.04),0_24px_60px_-36px_rgba(0,0,0,0.65)] backdrop-blur-xl"
        >
          <div className="flex items-center justify-between gap-3 border-b border-white/10 bg-white/[0.03] px-5 py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/45">Due today · open items</p>
              </div>
            </div>
            <Link href={ROUTES.va.tasks} className="text-xs font-medium text-pink-300/90 hover:text-pink-200">
              View all
            </Link>
          </div>
          <TaskRows
            tasks={todaysTasks}
            emptyHint="Nothing due today. Check all tasks for the rest of the week."
          />
        </motion.section>

        {/* 3. Work hours this week */}
        <motion.section
          variants={sectionItem}
          className="lg:col-span-2 flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-black/35 shadow-[0_0_0_1px_rgba(255,255,255,0.04),0_24px_60px_-36px_rgba(0,0,0,0.65)] backdrop-blur-xl"
        >
          <div className="flex items-center gap-3 border-b border-white/10 bg-white/[0.03] px-5 py-4">
            ...
            <div>
              ...
              <p className="text-xs text-white/45">{weekRangeLabel}</p>
            </div>
          </div>
          <div className="flex flex-1 flex-col gap-4 p-5">
            <div>
              <p className="text-xs font-medium uppercase tracking-wider text-white/40">This week</p>
              <p className="mt-1 text-3xl font-bold tabular-nums tracking-tight text-white">{weekHours}</p>
            </div>
            <div className="rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3">
              <p className="text-xs text-white/45">Today</p>
              <p className="mt-0.5 text-xl font-semibold tabular-nums text-pink-100/95">{todayHours}</p>
            </div>
            <VaShiftSummary data={shiftCardData} liveDurationMinutes={liveDurationMinutes} />
            <div className="mt-auto flex flex-col gap-2 border-t border-white/10 pt-4">
              <p className="text-[11px] text-white/40">
                All-time logged: <span className="font-medium text-white/60">{totalWorkedHours}</span>
              </p>
              <Link
                href={ROUTES.hours}
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/12 bg-white/[0.06] py-2.5 text-sm font-medium text-white/90 transition-colors hover:border-pink-400/30 hover:bg-pink-500/10"
              >
                <Calendar className="h-4 w-4 text-pink-300/90" aria-hidden />
                Hours &amp; reports
                <ArrowRight className="h-4 w-4 opacity-50" aria-hidden />
              </Link>
            </div>
      ...
      {/* 4. Pending reports (overdue open tasks) */}
      <motion.section
        variants={sectionItem}
        className="overflow-hidden rounded-2xl border border-white/10 bg-black/35 shadow-[0_0_0_1px_rgba(255,255,255,0.04),0_24px_60px_-36px_rgba(0,0,0,0.65)] backdrop-blur-xl"
      >
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 bg-white/[0.03] px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold text-white">Pending reports</h2>
              <p className="text-xs text-white/45">Open tasks past their due date — clear or reschedule on Tasks.</p>
            </div>
          </div>
          {pendingReports.length > 0 ? (
            <span className="rounded-full bg-rose-500/20 px-2.5 py-1 text-xs font-semibold text-rose-200 ring-1 ring-rose-400/30">
              {pendingReports.length} overdue
            </span>
          ) : null}
        </div>
        <TaskRows tasks={pendingReports} emptyHint="You're caught up — no overdue open tasks." />
      </motion.section>
