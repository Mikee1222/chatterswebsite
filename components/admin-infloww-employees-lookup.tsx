...
type StatusFilter = "activated" | "deactivated" | "deleted" | "all";
...
function matchesStatusFilter(status: string | undefined, filter: StatusFilter): boolean {
  if (filter === "all") return true;
  const n = normalizeStatus(status);
  if (filter === "activated") return n === "activated" || n === "active";
  if (filter === "deactivated") return n === "deactivated" || n === "inactive";
...
function statusBadgeClass(status?: string): string {
  ...
  if (n === "deactivated" || n === "inactive") {
    return "border-white/15 bg-white/10 text-white/55";
  }
  if (n === "deleted") {
    return "border-red-500/25 bg-red-500/10 text-red-300/70";
  }
...
export function AdminInflowwEmployeesLookup() {
  ...
  const [statusFilter, setStatusFilter] = React.useState<StatusFilter>("activated");
  ...
  const filtered = React.useMemo(() => {
    ...
    let rows = employees.filter((e) => matchesStatusFilter(e.status, statusFilter));
    if (q) {
  ...
  }, [employees, query, statusFilter, sortKey, sortDir]);
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/5">
      ...
      <div className="flex flex-col gap-3 border-b border-white/10 px-4 py-3 sm:flex-row sm:items-center">
        ...
        <label className="flex shrink-0 items-center gap-2 text-xs text-white/45">
          <span className="uppercase tracking-[0.1em]">Status</span>
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value as StatusFilter)}
            className="rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm text-white outline-none transition focus:border-[#FF1493]/50 focus:ring-1 focus:ring-[#FF1493]/20"
          >
      ...
      {!loading && !error && filtered.length === 0 ? (
        <p className="px-4 py-10 text-center text-sm text-white/40">
          {employees.length === 0
            ? "No employees returned from Infloww."
            ...
            <tbody>
              {pageRows.map((e) => (
                ...
                >
                  ...
                  <td className="px-4 py-2.5">
                    {e.status ? (
                      <span className={cn(VA_STATUS_BADGE, statusBadgeClass(e.status))}>
        ...
        <div className="flex flex-col gap-3 border-t border-white/10 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-white/35">
            ...
            {statusFilter !== "all" || query.trim() ? (
              <span className="text-white/25"> ({employees.length} total)</span>
            ) : null}
          </p>
          {filtered.length > 0 ? (
