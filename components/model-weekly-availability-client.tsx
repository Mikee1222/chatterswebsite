      ...
      <div className="space-y-8">
        ...
        >
          ...
          <p className="mt-1 text-white/60">
            {t(
              language,
              "Submit when you're available. You can add multiple entries per day (availability, day off, live window, custom window).", "Indica cuándo estás disponible. Puedes añadir varias entradas por día (disponibilidad, día libre, ventana en vivo, ventana personalizada)." )} </p> </div> {error && ( <div className="rounded-2xl border border-red-500/30 bg-red-500/10 px-5 py-4 text-sm text-red-200">{error}</div> )} {success && ( <div className="rounded-2xl border border-[hsl(330,80%,55%)]/30 bg-[hsl(330,80%,55%)]/10 px-5 py-4 text-sm text-[hsl(330,90%,75%)]"> {success} </div> )} <div className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-black/45 p-5 backdrop-blur-xl sm:flex-row sm:flex-wrap sm:items-center sm:justify-between" style={{ boxShadow: "0 0 0 1px rgba(255,255,255,0.05), 0 0 24px -8px hsl(330 80% 55% / 0.06)", }} > <div className="flex flex-wrap items-center gap-2"> <span className="mr-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-white/45"> {t(language, "Week", "Semana")} </span> <button type="button" onClick={() => goToWeek(-1)} className="inline-flex items-center gap-1.5 rounded-xl border border-white/14 bg-white/[0.06] px-3 py-2 text-sm font-medium text-white/90 shadow-sm transition-colors hover:border-white/22 hover:bg-white/10" > <ChevronLeft className="h-4 w-4 shrink-0 opacity-80" aria-hidden /> {t(language, "Previous", "Anterior")} </button> <button type="button" onClick={() => router.push(modelWeeklyAvailabilityUrl(getThisWeekMonday()))} className="rounded-xl border border-[hsl(330,80%,52%)]/45 bg-[hsl(330,80%,55%)]/18 px-4 py-2 text-sm font-semibold text-[hsl(330,92%,78%)] shadow-[0_0_20px_-6px_hsl(330_80%_55%/0.35)] transition-colors hover:bg-[hsl(330,80%,55%)]/28" > {t(language, "This week", "Esta semana")} </button> <button type="button" onClick={() => goToWeek(1)} className="inline-flex items-center gap-1.5 rounded-xl border border-white/14 bg-white/[0.06] px-3 py-2 text-sm font-medium text-white/90 shadow-sm transition-colors hover:border-white/22 hover:bg-white/10" > {t(language, "Next", "Siguiente")} <ChevronRight className="h-4 w-4 shrink-0 opacity-80" aria-hidden /> </button> </div> <div className="min-w-0 sm:max-w-[min(100%,28rem)] sm:text-right"> <p className="text-xs font-medium uppercase tracking-wider text-white/40">{t(language, "Week range (UTC)", "Rango semanal (UTC)")}</p> <p className="mt-0.5 text-sm font-semibold leading-snug tracking-tight text-white/90">{formatUtcWeekRangeLabel(weekStart, language)}</p> </div> <p className="w-full text-xs leading-relaxed text-white/45 sm:basis-full sm:text-right"> {t( language, "Pink dot on a day: period — content restrictions may apply.", "Punto rosa: día de período — pueden aplicarse restricciones." )} </p> </div> {requests.length > 0 && ( <div className="mb-4 flex items-center gap-2 rounded-2xl border border-green-500/20 bg-green-500/10 px-4 py-3 md:hidden"> <span className="text-sm font-semibold text-green-400"> {requests.length} {requests.length === 1 ? "entry" : "entries"} submitted this week </span> <span className="ml-auto text-xs text-green-400/50">Scroll down to see →</span> </div> )} <div className="grid gap-6 md:grid-cols-2"> <div className="flex min-h-[22rem] flex-col overflow-hidden rounded-2xl border border-white/10 bg-black/40 backdrop-blur-xl md:sticky md:top-4 md:self-start md:max-h-[calc(100vh-6rem)] md:order-2" style={{ boxShadow: "0 0 0 1px rgba(255,255,255,0.05), 0 0 24px -8px hsl(330 80% 55% / 0.06)", }} > <div className="flex flex-wrap items-end justify-between gap-2 border-b border-white/10 bg-white/[0.04] px-5 py-4"> <div> <h3 className="text-sm font-semibold uppercase tracking-wider text-white/90"> {t(language, "This week's submissions", "Envíos de esta semana")}
                </h3>
