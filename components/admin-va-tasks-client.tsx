...
import { AdminVaTasksViewToggle } from "@/components/admin-va-tasks-progress-overview";
import { AdminVaTasksPhasePanel } from "@/components/admin-va-tasks-phase-panel";
import {
  StaffAssigneePicker,
  staffDisplayName,
...
export function AdminVaTasksClient({
  ...
  const fillEditForm = React.useCallback(async (t: VaTaskRecord, scope: RecurringOccurrenceScope | null) => {
    ...
    try {
      ...
      const phases = normalizeTaskPhasesForClient(data.phases ?? []);
  ...
  const phaseResetKey = React.useMemo(
    () => [selectedYmd, rangeStartYmd, rangeEndYmd, deferredSearch, filterVa, filterStatus, filterPriority].join("|"),
  ...
  return (
    <div className="space-y-8">
      ...
      <AdminVaTasksPhasePanel
        viewMode={viewMode}
        canShowList={canShowList}
        canViewProgress={canViewProgress}
        canManage={canManage}
        visibleRegularTasks={visibleRegularTasks}
        regularTasks={regularTasks}
        recurringGroups={recurringGroups}
        showAllTasks={showAllTasks}
        onShowAllTasks={() => setShowAllTasks(true)}
        progressViewTasks={progressViewTasks}
        dateFilteredTasks={dateFilteredTasks}
        hasAnyTasks={localTasks.length > 0}
        vaUsers={vaUsers}
        staffUsers={staffUsers}
        nameById={nameById}
        reminding={reminding}
        remindSuccess={remindSuccess}
        confirmingTaskDelete={confirmingTaskDelete}
        taskPendingDelete={taskPendingDelete}
        onRemind={handleRemind}
        onEdit={openEdit}
        onDelete={handleDeleteRequest}
        expandedRecurringHistory={expandedRecurringHistory}
        onToggleRecurringHistory={toggleRecurringHistory}
        isSupabaseBackend={isSupabaseBackend}
